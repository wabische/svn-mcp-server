...
def svn_file_loc_export(
    ...
    if output_path:
        ...
        suffix = {"csv": ".csv", "tsv": ".tsv", "jsonl": ".jsonl"}.get(output_format, ".csv")
        out_file = os.path.join(os.path.dirname(base), f"_loc_export_{ts}{suffix}")
    ...
    for root, dirs, files in os.walk(base):
        ...
        for fn in files:
            ...
            # exclude_patterns チェック（glob、スラッシュ正規化）
            rel_fwd = rel.replace("\\", "/")
            if any(fnmatch.fnmatch(rel_fwd, pat.replace("\\", "/")) for pat in exclude):
                continue
            ...
            last_rev = ""
            if include_svn_revision:
                info_out, _ = run_svn(["info", "--xml", fp])
                info_root, _ = _parse_xml(info_out)
                if info_root is not None:
                    commit = info_root.find(".//commit")
                    if commit is not None:
                        last_rev = commit.get("revision", "")
            ...
            b = by_ext.setdefault(ext, {"files": 0, "total": 0, "code": 0, "comment": 0, "blank": 0})
            b["files"] += 1
            b["total"] += total
            b["code"] += code
            b["comment"] += comment
        ...
        if fmt == "jsonl":
            ...
            with open(out_file, "w", encoding="utf-8-sig", newline="") as f:
                writer = csv.DictWriter(
                    f,
                    fieldnames=["relative_path", "extension", "total", "code",
                                "comment", "blank", "size_bytes", "last_revision"],
                    delimiter=delim,
                )
                writer.writeheader()
                writer.writerows(rows)
    except Exception as ex:
        return {"error": f"failed to write output file: {ex}"}

    elapsed_ms = int((time.monotonic() - t0) * 1000)
    total_summary = {"files": 0, "total": 0, "code": 0, "comment": 0, "blank": 0}
    ...
    return {
        ...
        "total_code": total_summary["code"],
        "total_comment": total_summary["comment"],
        "total_blank": total_summary["blank"],
